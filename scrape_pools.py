...
def extract_pool_data(html, url):
    ...
    bassin_matches = re.findall(r'<div class=[\'"]places--pool-characteristics-description[\'"]>(.*?)</div>', html, re.DOTALL)
    ...
    for day in days:
        ...
        day_pattern = r'<div class="places--schedules-regular-content-weekday">\s*' + day + r'\s*</div>(.*?)<div class="places--schedules-regular-content-timerange">(.*?)</div>'
        ...
        if day_idx == -1:
             ...
             match_day = re.search(r'<div class="places--schedules-regular-content-weekday">\s*' + day + r'\s*</div>', html)
             if match_day:
        ...
        timerange_start = html.find('places--schedules-regular-content-timerange', day_idx)
        if timerange_start != -1:
            ...
            next_row = html.find('places--schedules-regular-content-row', timerange_start)
            if next_row == -1:
                next_row = len(html)
            ...
            sub_match = re.search(r'places--schedules-regular-content-exceptional-sub[^>]*>(.*?)</div>', timerange_html, re.DOTALL)
            if sub_match:
